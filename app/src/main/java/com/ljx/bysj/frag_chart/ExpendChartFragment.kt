...
class ExpendChartFragment : BaseChartFragment() {
    ...
    override fun setAxisData(year: Int, month: Int) {
        ...
        if (list.size == 0) {
            ...
            for (index in 0 until xLabelCount) {
                val entry = BarEntry(index.toFloat(), 0.0f)
...
//            barDataSet.setDrawValues(false) //是否显示柱子上面数值
    ...
    override fun setAxis(year: Int, month: Int) {
        ...
        xLabelCount = DBManage.getMaxOneDayInMonth(year, month, 0)
